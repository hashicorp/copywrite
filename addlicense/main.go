...
// extractYearRange extracts year1 and year2 from existing header for smart merging
func extractYearRange(b []byte) (string, string) {
	n := 1000
	if len(b) < 1000 {
		n = len(b)
	}
	...
	for _, line := range lines {
		...
		if strings.Contains(lowerLine, "copyright ibm corp") {
			...
			for i, part := range parts {
				if strings.ToLower(part) == "corp." && i+1 < len(parts) {
					yearPart := strings.TrimSuffix(parts[i+1], ",")
					if i+2 < len(parts) && strings.Contains(parts[i+2], "20") {
						// Format: "2017, 2025"
						return yearPart, parts[i+2]
					} else {
...
func updateLicense(path string, fmode os.FileMode, tmpl *template.Template, data LicenseData) (bool, error) {
	...
	var finalData LicenseData = data
...
// generateCopyrightLine creates the appropriate copyright line for the file type
func generateCopyrightLine(ext string, data LicenseData) string {
	copyrightText := fmt.Sprintf("Copyright %s %s", data.Holder, data.Year)
...
// removeExistingHeader removes an existing copyright/license header from the file content
func removeExistingHeader(content []byte, path string) ([]byte, error) {
	lines := bytes.Split(content, []byte("\n"))
	if len(lines) == 0 {
		return content, nil
	}
	...
	// Find the start and end of the header based on file type
	switch ext {
	case ".c", ".h", ".gv", ".java", ".scala", ".kt", ".kts", ".js", ".mjs", ".cjs", ".jsx", ".tsx", ".css", ".scss", ".sass", ".ts", ".gjs", ".gts":
		// Multi-line comment style /* */ or /** */
		for i, line := range lines {
			lineStr := string(bytes.TrimSpace(line))
			if !foundStart && (strings.HasPrefix(lineStr, "/*") || strings.HasPrefix(lineStr, "/**")) {
				startIdx = i
				foundStart = true
			}
			if foundStart && strings.HasSuffix(lineStr, "*/") {
				endIdx = i
				foundEnd = true
				break
			}
	...
	case ".cc", ".cpp", ".cs", ".go", ".hh", ".hpp", ".m", ".mm", ".proto", ".rs", ".swift", ".dart", ".groovy", ".v", ".sv", ".lr":
		...
		for i, line := range lines {
			...
			if strings.HasPrefix(lineStr, "//") {
				// Check if this line contains copyright or SPDX (start of header)
				if strings.Contains(strings.ToLower(lineStr), "copyright") || strings.Contains(strings.ToLower(lineStr), "spdx") {
					if !foundStart {
						startIdx = i
						foundStart = true
					}
					...
					// We're in a header block and found another comment line
					// Continue including it as part of the header to be removed
					endIdx = i
				}
			} else if foundStart && lineStr != "" {
				// Found a non-comment line, end the header block
				foundEnd = true
				break
			} else if foundStart && lineStr == "" {
				// Empty line during header - might be part of header or separator
				// Include it tentatively, but don't break
				endIdx = i
			}
		}
	...
	case ".html", ".htm", ".xml", ".vue", ".wxi", ".wxl", ".wxs":
		...
		for i, line := range lines {
			lineStr := string(bytes.TrimSpace(line))
			if !foundStart && strings.HasPrefix(lineStr, "<!--") {
				startIdx = i
				foundStart = true
			}
			if foundStart && strings.HasSuffix(lineStr, "-->") {
	...
	default:
		// For unknown file types, try to detect header block
		for i, line := range lines {
			lineStr := string(bytes.TrimSpace(line))
			if strings.Contains(strings.ToLower(lineStr), "copyright") || strings.Contains(strings.ToLower(lineStr), "spdx") {
				if !foundStart {
	...
	// If we found a header, remove it
	if foundStart {
		if !foundEnd {
			endIdx = len(lines) - 1
		}
		...
		newLines := make([][]byte, 0, len(lines)-(endIdx-startIdx+1))
		newLines = append(newLines, lines[:startIdx]...)

		// Skip any blank lines immediately after the header
		nextIdx := endIdx + 1
		for nextIdx < len(lines) && len(bytes.TrimSpace(lines[nextIdx])) == 0 {
			nextIdx++
		}
