...
const (
	defaultDispatchMaxAttempts = 15
	defaultDispatchSleep       = 10
	defaultDispatchWorkers     = 2
	defaultDispatchBranch      = "main"
	defaultDispatchBatchID     = ""
	defaultDispatchWorkflow    = "repair-repo-license.yml"
	defaultDispatchGitHubOrg   = "hashicorp"
)
...
func init() {
	...
	dispatchCmd.Flags().Int("max-attempts", defaultDispatchMaxAttempts, "Number of times a worker will check if a job has completed before timing out")
	dispatchCmd.Flags().IntP("sleep", "s", defaultDispatchSleep, "Seconds to sleep between polling opts")
	dispatchCmd.Flags().IntP("workers", "w", defaultDispatchWorkers, "Concurrent jobs that can be ran")
	dispatchCmd.Flags().StringP("branch", "b", defaultDispatchBranch, "The GitHub Branch to base workflow runs off of")
	dispatchCmd.Flags().StringP("batch-id", "i", defaultDispatchBatchID, "A unique identifier for the current batch of workflow runs (defaults to an autogenerated ULID)")
	dispatchCmd.Flags().StringP("workflow", "n", defaultDispatchWorkflow, "The workflow file name to be triggered")
	dispatchCmd.Flags().String("github-org", defaultDispatchGitHubOrg, "Sets the target GitHub org who's repos you wish to audit")
}
