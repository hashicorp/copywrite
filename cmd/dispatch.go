...
var dispatchCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		allRepos, err := repodata.GetRepos(githubOrgToAudit)
		cobra.CheckErr(err)
		...
		if len(conf.Dispatch.IgnoredRepos) > 0 {
			...
			targetRepos = lo.Filter(targetRepos, func(r *github.Repository, i int) bool {
				fqn := fmt.Sprintf("%v/%v", githubOrgToAudit, r.GetName())
		...
		if plan {
			...
			for _, v := range targetRepos {
				cliLogger.Info(fmt.Sprintf("%v/%v", githubOrgToAudit, *v.Name))
			}
...
func init() {
	...
	dispatchCmd.Flags().StringVar(&githubOrgToAudit, "github-org", "hashicorp", "Sets the target GitHub org who's repos you wish to audit")
}
